...
## [DONE] キャンペーン横断アーティファクトコレクション（重要アイテム記録） — 優先度: 低
**対象:** KP / 共通
...
## [DONE] コンバット・イニシアチブトラッカー（戦闘順序・HP管理） — 優先度: 高
**対象:** PL / KP / 共通
...
## [DONE] シナリオ探索ロケーション管理（場所メモ・訪問状況追跡） — 優先度: 中
**対象:** KP
...
## [DONE] クルー・コネクションボード（手がかり関係マップ） — 優先度: 中
**対象:** KP / 共通
...
## [DONE] パーティキャラクター関係図（人間関係ノードマップ） — 優先度: 低
**対象:** PL / 共通
...
## [TODO] KP活動実績ダッシュボード（主催シナリオ横断の個人統計） — 優先度: 高
**対象:** KP
**概要:** KPが自分の主催実績を一覧できる個人統計ページ。主催シナリオ数・累計参加プレイヤー数・シナリオ完了率・平均プレイヤー評価・最も好評だったシナリオをカード形式で表示し、KPとしての活動履歴とモチベーション維持を支援する。既存のシナリオ別統計（`/scenarios/[id]/stats/`）・キャンペーン別統計（`/campaigns/[id]/stats/`）とは異なる「KP個人のすべての主催実績の俯瞰ビュー」。
**実装ヒント:** `src/app/kp/dashboard/page.tsx` を Server Component で新規作成（認証必須）。`Promise.all` で ① `supabase.from("scenarios").select("id, title, status, played_at").eq("user_id", user.id)` でKPが作成した全シナリオを取得 → ② `supabase.from("scenario_participants").select("scenario_id, character_id").in("scenario_id", scenarioIds)` で累計参加者数を集計 → ③ `supabase.from("scenario_player_ratings").select("scenario_id, fun_rating, horror_rating, mystery_rating, character_rating").in("scenario_id", scenarioIds)` で各シナリオの平均評価を計算。集計値「主催シナリオ数・完了数・累計プレイヤー人数・平均評価」を `text-4xl font-bold` の大型カードで表示。完了済みシナリオの評価ランキング（上位3件）を「ベストシナリオ」セクションで表示。`src/app/kp/page.tsx` がなければ `src/app/kp/player-notes/page.tsx` と同じ階層に追加し、NavBar に「KP統計」リンクを追加。追加DBなし。
**コミット:** `feat: KP personal activity dashboard with cross-scenario hosting stats and ratings`

## [TODO] 後継者キャラクター継承ウィザード（前キャラからスキル・記憶引き継ぎ） — 優先度: 高
**対象:** PL
**概要:** 死亡・退場したキャラクターの後継者を作成する際に、前キャラクターの一部スキル値・秘密・形見品・PHOBIAを選択式で引き継ぐウィザード。`Character.successor_of: string | null` フィールドが既存だが未活用のため、後継者作成→前キャラとの紐付け→選択的引き継ぎ→成長履歴への記録というフローを実装する。CoC では「生存者が前任者の記憶を持つ」描写がよくあり、データ損失防止・RP継続性の両面で高優先度。
**実装ヒント:** `src/app/characters/[id]/inherit/page.tsx` を "use client" で新規作成（`id` は後継者キャラクターのID）。3ステップウィザード: ① 前キャラ選択（`supabase.from("characters").select("id, name, status").eq("user_id", user.id).in("status", ["dead","retired"])` で候補一覧表示） → ② 引き継ぎ項目選択（前キャラの `character_skills` を取得しチェックボックスで選択、選択したスキルは `Math.floor(oldValue * 0.5)` を初期値として後継者に付与。同様に `character_secrets`・`character_keepsakes`・`character_phobias` もチェックボックスで選択可） → ③ 確定・保存（`supabase.from("characters").update({ successor_of: predecessorId }).eq("id", successorId)` で紐付け保存、選択スキルを `character_skills` に INSERT し `growth_histories` に「[前キャラ名]からの引き継ぎ」レコードを追加）。後継者のキャラクター詳細ページ（`src/app/characters/[id]/page.tsx`）に `successor_of` が null でない場合「前任者: [name]」バッジを表示しリンク。前任者キャラクターの farewell ページに「後継者: [name]」表示も追加。追加DBなし（既存フィールドを活用）。
**コミット:** `feat: successor character inheritance wizard for skill and keepsake transfer from deceased characters`

## [TODO] AIセッション後PLフォローアップDM生成 — 優先度: 中
**対象:** KP
**概要:** セッション終了後にKPが各参加PLへ送る個別フォローアップメッセージをAIが自動生成するページ。各キャラクターのセッション体験（SAN喪失量・クリティカル成功/失敗・ハイライト・次回への伏線）を元にパーソナライズされた「お礼＋次回予告ミニDM」を生成し、Discord コピペで即送信できる。既存の「セッション要約生成」がKP向け全体まとめであるのに対し、本機能は「PL一人ひとりへの個別メッセージ」に特化する。
**実装ヒント:** `src/app/scenarios/[id]/follow-up/page.tsx` を "use client" で新規作成。① `supabase.from("scenario_participants").select("character_id, characters(name, player_name), players(display_name, contact_discord)").eq("scenario_id", id)` で参加者を取得 → ② 各参加者に対して `supabase.from("session_logs").select("san_loss, hp_loss, title").eq("character_id", charId).order("created_at", {ascending: false}).limit(1)` で最新セッションログを取得 → ③ `supabase.from("dice_rolls").select("skill_name, success_level").eq("character_id", charId).order("rolled_at", {ascending: false}).limit(20)` でセッション中のロール履歴を取得。これらを `src/app/api/ai/follow-up/route.ts`（新規・既存AIルートパターンを踏襲）に送信し「このPLのセッション体験（SAN喪失・最大の見せ場ロール・次回への引き・キャラ名）を踏まえ、KPからのアットホームなお礼DM文面を200字程度で生成」。各PL分を縦並びで表示し Discord コピー用ボタン（`navigator.clipboard.writeText`）を各メッセージに配置。`src/app/scenarios/[id]/page.tsx` の「セッション後」セクションに「💌 フォローアップ生成」リンクを追加。追加DBなし・追加APIルート1本。
**コミット:** `feat: AI post-session personalized follow-up DM generator for each PL`

## [TODO] AI難易度動的調整アドバイザー（セッション中リアルタイム補助） — 優先度: 中
**対象:** KP
**概要:** セッション進行中にパーティの現在HP/SAN/MPと残シナリオ内容・経過時間を入力するとAIが「このまま進むとどうなるか」を診断し、難易度を上下させる具体的な調整案（NPC強化/弱体化・ヒント追加・障害追加・タイムスキップ推奨など）を提案するページ。既存の `difficulty/page.tsx`（セッション前の難易度評価）や `hint-cards/page.tsx`（行き詰まり時のヒント生成）とは異なる「セッション中ペーシング調整」に特化したKPリアルタイム支援ツール。
**実装ヒント:** `src/app/scenarios/[id]/balance-advisor/page.tsx` を "use client" で新規作成。入力フォーム: ① パーティ現況（各参加キャラのHP/SAN/MP を `supabase.from("characters").select("name, hp_current, hp_max, san_current, san_max, mp_current, mp_max").in("id", participantIds)` で自動取得しスライダーで手動上書き可） ② 進行フェーズ（「導入/調査/クライマックス/終盤」のラジオボタン） ③ 経過時間（分単位入力） ④ KPの主観評価（「PL体力に余裕あり/ちょうどよい/PLがしんどそう」の3択）。これらを `src/app/api/ai/balance-advisor/route.ts`（新規）に送信: 「以下のパーティ状況・進行フェーズ・経過時間・KP評価から、現在の難易度バランスを診断し具体的な調整案を『強化系（敵強化・障害追加）』と『緩和系（NPC援助・情報開示・時間短縮）』にそれぞれ2件ずつJSON形式で提案してください。各提案に実施タイミングと期待効果を含めること。」結果を「🔥 強化案」「💧 緩和案」の2カラムで表示し、各提案にKPメモ追加ボタンを配置（`supabase.from("scenario_notes").insert`）。`src/app/scenarios/[id]/kp-dashboard/page.tsx` に「⚖️ 難易度アドバイザー」ショートカットリンクを追加。追加DBなし・追加APIルート1本。
**コミット:** `feat: AI real-time difficulty balance advisor for KP mid-session pacing adjustment`
