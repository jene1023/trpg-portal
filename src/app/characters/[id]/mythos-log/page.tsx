...
export default function MythosLogPage() {
  const params = useParams();
  const id = params.id as string;

  const [encounters, setEncounters] = useState<CharacterMythosEncounter[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const [{ data: char }, { data: encs }, { data: creaturesData }] = await Promise.all([
  ...
  const addEncounter = useCallback(async () => {
    ...
    setEncounteredAt("");
    setSaving(false);
  }, [id, useCreatureRef, creatures, selectedCreatureId, entityName, entityType, sessionLabel, sanLost, notes, encounteredAt]);
  ...
  return (
    <div className="coc-page-enter mx-auto max-w-2xl px-4 py-8 space-y-6">
      ...
      <div>
        <h1 className="font-cinzel text-2xl font-bold text-coc-text coc-name-glow">神話遭遇ログ</h1>
            ...
            {entitySummaries.map((summary) => (
              <div key={summary.entity_name} className="rounded-lg border border-coc-border bg-coc-raised overflow-hidden">
                <button
                  ...
                  onClick={() =>
                    setExpandedEntity(
                      expandedEntity === summary.entity_name ? null : summary.entity_name
                    )
                  }
                >
                  <div className="flex items-center gap-2 text-left min-w-0">
                    <span
                      className={`shrink-0 rounded border px-1.5 py-0.5 text-xs font-semibold ${entityTypeBadgeClass(summary.entity_type)}`}
                    >
