...
import { ArrowLeft, Users, FileText, User, Shield, StickyNote, Swords, CalendarClock, ShieldCheck, ClipboardList, BarChart2, MapPin, Vote, Bug, Music, ListChecks, Star, Clock, ExternalLink, UserCheck, Monitor, Radio, Skull, Package, Dices, MessageSquare, BookOpen, HelpCircle, TimerIcon, PlayCircle, TrendingUp, PenLine, Film, Trophy, AlertTriangle, MessageSquarePlus, ShieldAlert, UserPlus, Search, GitBranch, Gauge, Download, Library, ScrollText, Archive, UserMinus, CheckSquare, Users2, Megaphone, Activity, Share2, RefreshCw, Target, BookMarked, ClipboardCheck, Heart, Sparkles, Network, Lightbulb } from "lucide-react";
  ...
  return (
    <div className="coc-page-enter mx-auto max-w-2xl px-4 py-8">
      ...
      <div className="flex flex-col gap-3 mb-6">
        ...
        <Link
          href={`/scenarios/${id}/chase`}
          className="flex items-center justify-between rounded-xl border border-coc-border bg-coc-surface px-5 py-4 hover:border-coc-gold transition-colors"
        >
          <div className="flex items-center gap-3">
            <Activity size={20} className="text-coc-gold" />
            <div>
              <p className="font-medium text-coc-text">🏃 チェイストラッカー</p>
              <p className="text-xs text-coc-muted">SPD順・距離カウンター・逃走/捕捉判定（CoC 7版チェイスルール専用）</p>
            </div>
          </div>
          <span className="text-coc-muted">→</span>
        </Link>
        ...
        <Link
          href={`/scenarios/${id}/hint-cards`}
          className="flex items-center justify-between rounded-xl border border-coc-gold-dim bg-coc-raised px-5 py-4 hover:border-coc-gold transition-colors"
        >
          <div className="flex items-center gap-3">
            <Lightbulb size={20} className="text-coc-gold" />
            <div>
              <p className="font-medium text-coc-gold">💡 ヒントカード</p>
              <p className="text-xs text-coc-muted">行き詰まったPLへの段階的ヒントを3段階でAI生成し、ワンクリックで開示</p>
            </div>
          </div>
          <span className="text-coc-muted">→</span>
        </Link>
